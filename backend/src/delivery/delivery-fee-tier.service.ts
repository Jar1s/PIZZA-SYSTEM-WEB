...
export class DeliveryFeeTierService {
  ...
  async getDeliveryFeeByDistance(
    ...
    // Check if distance is beyond max tier (mimo dosahu)
    if (tiers.length > 0) {
      const maxTier = tiers.reduce((max, tier) => 
        tier.maxDistanceMeters > max.maxDistanceMeters ? tier : max
      );
      
      if (distanceMeters > maxTier.maxDistanceMeters) {
        this.logger.warn('Address is outside delivery range', {
          tenantId,
          distanceMeters,
          maxDistanceMeters: maxTier.maxDistanceMeters,
        });
        // Return result with isOutOfRange flag so controller can distinguish
        return {
          deliveryFeeCents: 0, // Not used when out of range
          distanceMeters,
          isOutOfRange: true,
        };
      }
    ...
    // No tier matched but within range - use defaultFeeCents if available
    const defaultFeeCents = deliveryConfig?.defaultFeeCents;
    if (defaultFeeCents !== undefined && typeof defaultFeeCents === 'number') {
      this.logger.warn('No tier matched distance, using default fee', {
        tenantId,
        distanceMeters,
        defaultFeeCents,
      });
    ...
    // No tier matched and no default fee
    this.logger.warn('No tier matched distance and no default fee configured', {
      tenantId,
      distanceMeters,
    });
    return null;
