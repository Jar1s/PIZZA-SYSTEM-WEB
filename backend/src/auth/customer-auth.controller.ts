...
const logger = new Logger('CustomerAuthController');
...
function getOAuthCookieOptions(frontendUrl: string) {
  ...
  if (!domain) {
    try {
      ...
      logger.error(
        `Error parsing frontend URL for cookie domain: ${(error as Error)?.message || 'unknown error'}`,
  ...
  logger.log(`OAuth cookie options prepared: ${JSON.stringify({
    domain,
...
export class CustomerAuthController {
  ...
  async googleRedirect(
    ...
    if (!redirectUri) {
      logger.error('Google OAuth redirect URI not configured. BACKEND_URL or GOOGLE_REDIRECT_URI must be set.');
    ...
    logger.log(`Google OAuth redirect URI: ${redirectUri}`);
    logger.log(`Google OAuth config: ${JSON.stringify({
      redirectUri,
      frontendUrl,
    ...
    if (process.env.NODE_ENV === 'production' && redirectUri.includes('localhost')) {
      logger.error(`Google OAuth redirect URI contains localhost in production: ${JSON.stringify({
        redirectUri,
        GOOGLE_REDIRECT_URI: process.env.GOOGLE_REDIRECT_URI,
        FRONTEND_URL: process.env.FRONTEND_URL,
      })}`);
    ...
    logger.log(`Redirecting to Google OAuth with redirect URI: ${redirectUri}`);
  ...
  async googleExchange(
    ...
    try {
      ...
      logger.log(`Google OAuth exchange using redirect URI: ${redirectUri}`);
      ...
      logger.error(
        `Google OAuth exchange error: ${error?.message || 'unknown error'}`,
        error?.stack,
      );
      return res.status(500).json({
  ...
  async googleCallback(
    ...
    try {
      ...
      if (!redirectUri) {
        logger.error('Google OAuth callback: redirect URI not configured. BACKEND_URL or GOOGLE_REDIRECT_URI must be set.');
        const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3001';
      ...
      // Warn if using localhost in production
      if (process.env.NODE_ENV === 'production' && redirectUri.includes('localhost')) {
        logger.error(`Google OAuth callback redirect URI contains localhost in production: ${JSON.stringify({
          redirectUri,
          backendUrl,
          GOOGLE_REDIRECT_URI: process.env.GOOGLE_REDIRECT_URI,
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        logger.log(`OAuth callback mode: ${JSON.stringify({
          frontendUrl,
          useUrlParams: true,
          NODE_ENV: process.env.NODE_ENV,
        ...
        if (useUrlParams) {
          ...
          if (result.needsSmsVerification) {
            ...
            if (returnUrl) {
              ...
              redirectUrl = tenant ? `/account?tenant=${tenant}` : '/account';
            }
          }
          
          logger.log(`Google OAuth callback redirect prepared: ${JSON.stringify({
            redirectUrl,
            needsSmsVerification: result.needsSmsVerification,
          })}`);
          ...
          logger.log(`Setting OAuth cookies with options: ${JSON.stringify(oauthCookieOptions)}`);
          ...
          logger.log('OAuth cookies set successfully');
          ...
          logger.log(`Google OAuth callback (prod) redirect: ${JSON.stringify({
            oauthCallbackUrl,
            redirectUrl,
            frontendUrl,
            ...
            userEmail: redactEmail(result.user.email),
            needsSmsVerification: result.needsSmsVerification,
          })}`);
      ...
      logger.error(
        `Google OAuth callback error: ${error?.message || 'unknown error'}`,
        error?.stack,
      );
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3001';
    ...
    try {
      ...
      if (userParam) {
        try {
          ...
          logger.warn(`Failed to parse Apple user parameter: ${(e as Error)?.message || 'unknown error'}`);
        }
      }
      ...
      logger.log(`Apple OAuth callback mode: ${JSON.stringify({
        frontendUrl,
        useUrlParams: true,
        NODE_ENV: process.env.NODE_ENV,
        reason: 'Using URL params for cross-domain compatibility'
      ...
      if (useUrlParams) {
        ...
        logger.log(`Apple OAuth callback redirect prepared: ${JSON.stringify({
          redirectUrl,
          needsSmsVerification: result.needsSmsVerification,
        })}`);
      ...
      if (result.needsSmsVerification) {
        ...
        if (returnUrl) {
          ...
          redirectUrl = tenant ? `/checkout?tenant=${tenant}` : '/checkout';
        }
      }
      ...
      logger.error(
        `Apple OAuth callback error: ${error?.message || 'unknown error'}`,
        error?.stack,
      );
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3001';
      res.redirect(`${frontendUrl}/auth/login?error=${encodeURIComponent(error.message || 'oauth_failed')}`);
