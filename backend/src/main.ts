...
async function bootstrap() {
  ...
  // Validate JWT_SECRET at startup (required by JwtStrategy)
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    if (req.method === 'OPTIONS') {
      ...
      const allowed = appConfig.isCorsOriginAllowed(origin);
      if (allowed) {
  ...
  app.enableCors({
    origin: (origin, callback) => {
      callback(null, appConfig.isCorsOriginAllowed(origin));
    },
    credentials: true,
