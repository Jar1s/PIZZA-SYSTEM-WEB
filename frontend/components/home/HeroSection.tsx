...
export const HeroSection = ({ tenantName, primaryColor, isDark = false }: HeroSectionProps) => {
  ...
  return (
    <section className="relative min-h-screen sm:min-h-[600px] md:min-h-[660px] overflow-hidden" style={{ position: 'relative', zIndex: 10 }}>
      {/* Background Image */}
      ...
      {/* Content */}
      <div className="relative z-10 container mx-auto px-4 sm:px-6 h-full min-h-screen sm:min-h-0 py-16 sm:py-12 md:py-16 lg:py-20 flex flex-col justify-between sm:justify-center">
        <motion.div
          initial={{ opacity: 0, x: -30 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.5 }}
          className="max-w-3xl text-white w-full flex-1 flex flex-col justify-center pt-8 sm:pt-0"
        >
          <motion.span
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.05, duration: 0.4 }}
            className={`${isDark ? 'hero-badge mb-6 sm:mb-6 inline-flex' : 'inline-flex items-center gap-1.5 sm:gap-2 rounded-full bg-white/90 sm:bg-white/80 px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-semibold text-black mb-8 sm:mb-6'}`}
          >
          ...
          <motion.h1
            ...
            className="text-4xl sm:text-4xl md:text-5xl lg:text-7xl font-black mb-6 sm:mb-4 md:mb-6 leading-[1.1] sm:leading-tight"
            style={{
              textShadow: 'none',
          ...
          <motion.p
            ...
            className="text-base sm:text-base md:text-lg lg:text-xl mb-8 sm:mb-6 max-w-2xl leading-relaxed"
            style={{
          ...
          <motion.div
            ...
            className="flex flex-wrap gap-3 sm:gap-4 items-center mb-12 sm:mb-0"
          >
            <button
        ...
        <motion.div
          ...
          className="mt-auto pb-8 sm:pb-0 sm:mt-10 grid grid-cols-3 gap-2.5 sm:gap-4"
        >
          {stats.map((stat) => (
            <div
              key={stat.label}
